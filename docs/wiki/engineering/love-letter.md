...
사전 등록과 설정 변경은 같은 축제 잠금을 사용하고, 사전 등록은 잠금 획득 후 현재
동의문 버전을 다시 확인한다. 링크 연결은 초대 소비·기존 브라우저 토큰 해제·새 토큰
귀속·배정을 한 transaction에서 처리해 이전 쿠키의 대기 중 등록 요청을 차단한다.
...
행사 종료 후 공개 열람을 닫고 정확히 7일이 지난 시점부터 기존 cleanup 배치에서 쪽지·배정·
