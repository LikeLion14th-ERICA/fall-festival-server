...
제약으로 보호한다. 등록 transaction은 암호화 쪽지와 `PENDING` 당일 기록을 함께 저장한다.
쪽지 `created_at + 60초` 전에는 추첨을 거절한다. 그 후 축제별 transaction lock 아래
다른 성별, 미배정·미차단·타인 쪽지를 무작위 선정하고 배정과 `COMPLETED` 전환을 함께
저장한다. 후보가 없으면 `PENDING`을 유지하고 409를 반환한다. 사전 등록자도 지정일에
같은 배정 잠금·60초 규칙을 따른다. Idempotency-Key는 참여자와 요청 hash에 묶어
등록에는 쪽지 ID·가능 시각, 추첨에는 결과 ID만 복구한다. 연락처 원문은 저장하지 않는다.
...
| `GET /api/v2/love-letter-status` | 오늘 참여·60초 대기·추첨 가능 시각·봉투 상태·최근 연락처 |
| `POST /api/v2/love-letters` | 쪽지 등록, `drawAvailableAt` 반환 |
| `POST /api/v2/love-letter-draws` | 60초 후 추첨, 결과 ID만 반환 |
| `POST /api/v2/love-letter-results/{id}/open` | 해당 브라우저에게만 봉투 원문 공개 |
| `POST /api/v2/love-letter-results/{id}/reports` | 현재 받은 쪽지 신고 |
| `POST /api/v2/love-letter-invitations/claim` | 일회용 사전 링크 연결 |
| `POST /api/v2/love-letter-seeded-draws` | 지정일 사전 참여자 추첨 |
...
요청에 Origin 검증과 CSRF 방어를 적용한다. 등록·추첨·신고·링크 연결에 요청 수 제한을
