...
- API v1은 기존 공개 앱·관리자 도구의 계약 기록이다. 새 화면의 목·연동 계약은 API v2
  draft.3이며, 공개 카탈로그와 관리자 인증 경로는 서버에 구현했다. 나머지 경로를 운영으로
  전환하기 전에는 적용 버전과 v1 호환 전략을 승인된 결정 기록으로 확정한다. 구현 기술 스택은
  계약과 분리한다.
...
2026-09-14 Product 위키 v5·사용자 결정 기준 새 계약은 [API v2](../../../api-v2/README.md)를 사용합니다.
[OpenAPI](../../../api-v2/openapi.json), [화면 추적표](../../../api-v2/SCREEN-DATA.md),
[목 실행 안내](../../../api-v2/FRONTEND.md)를 함께 확인합니다. 위 v1 설명은 기존 계약의 기록이며
v2의 공지·굿즈·스탬프 범위에 그대로 적용하지 않습니다. v1 구현과 경로는 변경하지 않았습니다.
기술 초안과 미확정 운영 자료는 [v2 결정 대기](../../../api-v2/DECISIONS.md)에 구분합니다.
