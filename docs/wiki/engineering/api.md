...
API v2는 공개 앱과 관리자 운영 도구가 공유하는 단일 HTTP 계약이다. 구현 기술 스택과
독립적으로 URI, 요청·응답, 인증 경계, 오류, pagination, locale, 시간대,
캐시·동시성 규칙을 정의한다. 구현은 같은 계약의 OpenAPI artifact와 계약 테스트를
함께 갱신한다.
...
- 단일 무대의 공연, 공연 상태, 콘텐츠 게시 상태, 구조화 메뉴, 의무실과 공연장 전체를
  표현할 수 있어야 한다.
- 이미 공개한 v2 필드의 제거, 의미 변경, enum 재해석은 영향받는 소비자와
...
- API v2가 제품의 유일한 API 계약이다. 공개 카탈로그와 관리자 인증 경로는 서버에
  구현했고, 나머지 경로는 OpenAPI와 목 계약을 기준으로 운영 전환 범위를 확정한다.
  구현 기술 스택은 계약과 분리한다.
- API 요청·응답·enum·오류·페이지네이션을 변경하는 PR은 같은 PR에서 명세와 계약
  테스트를 갱신한다.
- 기존 v2 소비자를 깨는 필드 제거·의미 변경·enum 재해석은 영향 분석과
  마이그레이션 계획 없이 배포하지 않는다.
...
2026-09-14 Product 위키 v5·사용자 결정 기준 제품 계약은 [API v2](../../../api-v2/README.md)를 사용합니다.
