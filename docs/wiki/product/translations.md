...
- 부스 분류: 번역·디자인은 `주점/부스/플리마켓/푸드트럭/총학생회 부스/프로모션 부스` 6개,
  명세([BOOTH-001](spaces.md))와 API는 `전체/부스/플리마켓/주점` 3개 유형이다.
- 지도 필터: 번역·디자인은 `화장실/포토부스/흡연구역/쓰레기통` 칩, API 필터 그룹은
  `STUDENT_COUNCIL/EXPERIENCE/CONVENIENCE/FOOD_AND_BEVERAGE/PERFORMANCE`다.
  디자인 칩 목록에 `포토부스`가 두 번 있다.
- 지도 구역: 번역은 푸드트럭·주점존을 `체육관 앞 · 학술정보관 주차장`으로 적었으나
  [학교 용어](terminology.md)는 `예체능대학`으로 확정했다. 번역의 `체육관`, `잔디광장`,
  `호수공원 → Lions' Lake`는 용어표에 아직 없다.
