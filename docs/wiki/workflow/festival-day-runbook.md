...
실행 담당자는 팀에서 주 담당, 대체 담당과 연락 경로를 확정해 채운다. 같은 사람이 여러 역할을
겸할 수 있다.

| 역할 | 주 담당 | 대체 담당 | 연락 경로 | 권한 필요 |
|---|---|---|---|---|
| 혼잡도 입력 | (미정) | (미정) | (미정) | `ADMIN` 권한 관리자 계정, access JWT |
| 공지 게시·수정·삭제 | (미정) | (미정) | (미정) | 관리자 계정 |
| 굿즈 재고(판매중/품절) 전환 | (미정) | (미정) | (미정) | 관리자 계정 |
| 굿즈 상품 등록·수정 | (미정) | (미정) | (미정) | 관리자 계정 |
| 계좌 교체(CLI) | (미정) | (미정) | (미정) | DB account operator role, 서버 실행 환경 접근 |
| 스탬프 코드 교체 | (미정) | (미정) | (미정) | 서버 환경변수·재시작 권한 |
| catalog 게시·rollback(CLI) | (미정) | (미정) | (미정) | DB catalog publish role |
| A1 서버 재시작·장애 대응 1차 | (미정) | (미정) | (미정) | A1 접근 권한 (제원 님 또는 위임자) |
...
- [ ] 이 문서의 역할 분담표에 모든 역할의 주 담당·대체 담당·연락 경로가 채워져 있다.
