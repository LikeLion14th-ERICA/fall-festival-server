...
- Home에서 공지, 스탬프투어, 외부인 티켓과 외부 채널로 이어지는 진입 흐름을 확인할 수 있다.
...
- 목록, 상세, 옵션별 품절 상태, 송금 안내와 재고 조회 오류의 표시 흐름을 확인할 수 있다.
...
- `HOME-005` FAQ: Home에는 외부 이동처럼 표시되어 있지만 내부 FAQ 화면과 이동 방식은
  Product와 Figma 모두 미확정이다. 독립 Frame도 확인되지 않았다.
- 에리카 웰컴 데이: Home의 메뉴 위치만 있고 기능 ID와 독립 Frame은 없다. Product 명세가
  마련되기 전에는 구현 범위로 보지 않는다.
