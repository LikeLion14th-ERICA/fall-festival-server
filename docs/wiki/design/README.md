...
- Home에서 공지, 스탬프투어, 외부인 티켓과 외부 채널로 이어지는 진입 흐름을 확인할 수
  있다. 에리카 웰컴 데이는 2026-09-22 제거했다.
...
- 경품 수령 안내는 `상품 수령` 버튼이 아닌 수령 인증 코드 입력칸과 `확인` 버튼으로
  구현한다. 현장 담당자가 사용자 기기에 코드를 입력하는 흐름은 `STAMP-001`이 기준이다.
...
- `WELCOME-001`(제거됨): Home Frame `438:23`에 이전 진입 UI가 남아 있을 수 있으나
  2026-09-22 결정으로 기능 자체를 없앴다. 디자인 쪽에서도 해당 진입점을 빼야 한다.
