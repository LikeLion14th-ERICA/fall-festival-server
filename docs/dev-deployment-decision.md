...
PUBLIC_LOCALES=ko
RATE_LIMIT_TRUSTED_PROXY_HOPS=<frontend-proxy-and-load-balancer-count>
```

`API_DOCS_ENABLED=true`는 개발 서버에서만 켠다(`/docs` Swagger UI). `PUBLIC_LOCALES`는 번역이
모두 들어간 revision을 게시한 뒤에만 `ko,en,zh-Hans`처럼 늘리고, 재시작 로그의
`Published locales`로 실제 공개 언어를 확인한다. `RATE_LIMIT_TRUSTED_PROXY_HOPS`는 실제 앞단 구성
(Next.js proxy, 호스팅 load balancer)을 확인해 정한다. 틀리면 모든 사용자가 한 client로 묶이거나
`X-Forwarded-For` 위조를 믿게 된다. `STAMP_RECEIPT_CODE_SHA256`은 운영자가 코드를 정한 뒤 hash만
secret으로 넣는다. 그 밖의 선택 설정은 README 환경변수 표를 따른다.
