...
export const schemas = {
  ...
  Notice: object({ id, type: enumeration(['GENERAL', 'LOST_FOUND'], '일반은 당일 등록분, 분실물은 날짜와 무관'), contentLocale: enumeration(['ko', 'en', 'zh-Hans', 'ja'], '실제 응답에 쓰인 언어. ko→ko. en→en, 과거 자료에 en 번역이 없으면 ko. zh-Hans·ja→이 공지에 해당 번역이 있을 때만 사용, 없으면 en, 그다음 ko. 필드별 혼합 대체는 하지 않음.'), title: text('전체 제목'), body: text('전체 본문, plain text. HTML 실행 금지.'), links: array(ref('Link'), '본문 외부 링크. label은 응답의 contentLocale과 같은 언어. 이미지를 직접 표시하는 필드 없음.'), createdAt: timestamp }),
  ...
  GoodsImage: object({ alt: text('상품의 contentLocale과 같은 언어로 해석된 이미지 대체 텍스트'), masterUrl: text('same-origin master WebP API path', { format: 'uri-reference' }), thumbnail320Url: text('same-origin 320px WebP API path', { format: 'uri-reference' }), thumbnail640Url: text('same-origin 640px WebP API path', { format: 'uri-reference' }) }),
  Goods: object({ id, contentLocale: enumeration(['ko', 'en', 'zh-Hans', 'ja'], '실제 응답에 쓰인 언어. 공지와 같은 대체 규칙.'), name: text('상품명'), description: optionalText, price: ref('Money'), optionMode: enumeration(['SINGLE', 'OPTIONS'], 'SINGLE은 단일 판매 상태, OPTIONS는 색상×사이즈 조합별 판매 상태'), images: array(ref('GoodsImage'), 'sort_order 순 상품 이미지. 기존 이미지 없는 상품은 [].'), colors: array(ref('GoodsColor'), 'SINGLE이면 []'), sizes: array(ref('GoodsSize'), 'SINGLE이면 []') }),
  ...
  Availability: object({ goodsId: id, name: text('상품명. 관리자 목록 표시용.'), combinations: array(ref('GoodsCombinationStatus'), '색상×사이즈 조합별 또는 SINGLE 단일 판매 상태. 수량 필드 없음.'), allSoldOut: bool('조합이 1개 이상이고 모두 SOLD_OUT일 때 true'), updatedAt: nullable(timestamp, '마지막 저장 성공 시각') }),
  GoodsList: object({ items: array(ref('Goods'), '상품 전체. 품절 상품도 유지.') }),
  AvailabilityList: object({ items: array(ref('Availability'), '상품 전체의 조합 상태. 실패 시 상품 목록과 독립적으로 오류 처리.') }),
  BankAccount: object({ bankName: text('은행명'), accountNumber: text('계좌 문자열. 목 값은 송금할 수 없는 MOCK-NOT-PAYABLE.'), holder: text('예금주') }),
  PaymentGuide: object({ goodsId: id, name: text('상품명'), price: ref('Money'), account: nullable(ref('BankAccount'), '운영 계좌 CLI로 승인된 GOODS 계좌. 미승인이면 null. 관리자 웹에서는 변경 불가.'), transferLink: nullable(ref('Link'), '승인된 계좌 설정의 송금 링크. 없으면 null.'), instructions: array(text('안내 문구'), '현장 확인·송금·지급 안내. 입금/지급 완료 상태 없음.'), locationText: optionalText, hoursText: optionalText }),
