...
export const schemas = {
  ...
  Space: object({ id, category: enumeration(['BOOTH', 'PUB', 'FLEA_MARKET'], '부스/주점/플리마켓'), name: text('장소명'), image: ref('Image'), locationText: text('목록 필수 위치 안내'), operator: optionalText, hoursText: optionalText, description: optionalText, contact: nullable(ref('Link'), '없으면 영역 숨김'), experience: optionalText, events: array(text('부스 이벤트'), '부스만, 없으면 []'), menu: array(object({ name: text('메뉴명'), price: ref('Money') }), '주점만. 미확정 가격 메뉴의 노출 정책은 별도 결정.'), mapTarget: nullable(ref('MapTarget'), mapTargetDescription) }),
