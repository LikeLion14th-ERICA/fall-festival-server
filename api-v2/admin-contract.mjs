...
export function applyAdminContract(s,ops){
  ...
  s.Goods.properties.options=arr(obj({colorId:id,sizeId:id}),'실제 제공하는 조합만 명시. 자동 곱집합 생성 없음.');
  s.Goods.required.push('options');
  s.Goods.properties.images=arr(ref('Image'),'상품 복수 이미지. 배열 순서대로 보존, 실제 배치는 GOODS-001에서 정의.');
  s.Goods.properties.colors=arr(obj({id,name:str('색상 이름'),images:arr(ref('Image'),'선택 색상별 이미지')}),'등록 색상. 실제 제공 조합은 options에 별도 등록');
  ...
  s.ProductInput=obj({name:str('필수 상품명'),price:ref('Money'),images:arr(ref('Image'),'복수 이미지, URL 등록 방식은 목용 제안'),colors:s.Goods.properties.colors,sizes:s.Goods.properties.sizes,options:s.Goods.properties.options,description:nullable(str('상품 소개'),'선택 소개')},'상품과 실제 제공 조합 저장. 기존 상태 보존. 신규 조합 초기 상태·옵션 삭제 정책은 미정. 목의 신규 옵션 시나리오를 명시한 경우에만 생성 가능.');
  ...
  add('postAdminProduct','POST','/admin/products','Goods','상품 등록·초기 판매 상태 미정',['ADM-GOODS-PRODUCT-EDIT'],'ProductInput',['normal','new-option-on-sale','new-option-sold-out','missing-optional','error'],true);
  add('putAdminProduct','PUT','/admin/products/{goodsId}','Goods','상품 수정·기존 조합 판매 상태 유지',['ADM-GOODS-PRODUCT-EDIT'],'ProductInput',['normal','new-option','new-option-on-sale','new-option-sold-out','option-removal','not-found','error'],true);
