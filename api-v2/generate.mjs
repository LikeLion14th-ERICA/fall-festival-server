...
for(const op of operations){
  ...
  const responses=Object.fromEntries(statuses.map(status=>{
    ...
    const cacheControlHeaders=op.cacheControl&&(status<300||(op.conditional&&status===304)||LOVE_LETTER_OPERATION_IDS.includes(op.operationId))?{'Cache-Control':{schema:{type:'string',enum:[op.cacheControl]},description:op.cacheControl==='no-store'?'브라우저와 중간 캐시가 응답을 저장하지 못하게 한다.':op.binaryResponse?'응답에 적용되는 캐시 지시문.':'공유 캐시 금지와 매 요청 재검증. proxy는 이 값과 ETag를 그대로 전달한다.'}}:{};
