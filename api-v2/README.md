...
- 지도 이미지와 핀 요청을 분리합니다. 핀 요청의 `mapVersion`은 필수이며 이미지 버전 불일치는 409입니다. 구역 이동과 장소 팝업 대상은 서로 다른 타입입니다. 핀은 세부 `category`와 대분류 `filterGroup`을 함께 내보내고, `AREA` 핀은 `filterGroup: null`로 항상 표시합니다.
