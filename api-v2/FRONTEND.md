...
Hyped는 `ARTIST`만 대상으로 하며 `CONTEST`는 제외합니다. 축제 회차와 아티스트별 공유 누적
수로, 로그인·개인 식별·개인별 횟수 제한을 사용하지 않습니다. 각 명시적 클릭을 하나의 공개
쓰기 요청으로 보내며, 반복 클릭도 허용합니다.

동적 상태와 count는 상대 경로의 same-origin proxy를 통해 조회합니다. 성공 응답은 표준
`{ data, meta }` envelope를 사용하며 GET의 `data` payload는 다음과 같습니다.

```json
{
  "hypedEnabled": true,
  "items": [{ "artistId": "<artist-id>", "hypedCount": 12000 }]
}
```

`GET /api/v2/artist-hyped`는 읽기 전용이며 축제일 전후에도 count를 조회할 수 있습니다.
`items`의 `artistId`로 lineup·상세의 안정된 아티스트 ID와 연결합니다. `hypedEnabled`가
`true`일 때만 참여를 열고, 서버가 최종 허용 여부를 검사합니다. 참여 시간은 각 등록된
`FestivalDay`의 Asia/Seoul 기준 00:00부터 다음 날 00:00 직전까지다. 각 날짜의 공연 시간과
관계없이 하루 전체를 허용합니다. 그 외 시간에는 count만 읽을 수 있습니다.

출연진 상세에서 `POST /api/v2/artists/{artistId}/hyped`에 JSON `{}`를 보냅니다. 성공 응답의
`data`는 `{ "artistId": "<artist-id>", "hypedCount": 12001 }` 형태이며 서버가 반영한
공유 누적 수를 표시합니다. POST는 로그인 없이 호출하며 브라우저·기기별 참여 제한을 두지
않습니다. 타임아웃이나 연결 종료처럼 결과가 모호하면 자동으로 다시 보내지 않습니다.
표시 count는 낙관적으로 늘리지 않고, 모호한 요청은 다음 GET에서 동기화합니다.

두 경로의 응답은 `Cache-Control: no-store`를 사용합니다. 브라우저 요청은 `/api/v2`로
보내고 same-origin proxy가 해당 응답 헤더를 보존하며 캐시하지 않게 합니다.

화면 진입·재활성화·온라인 복귀 때 즉시 GET을 요청한 뒤, 화면이 보이고 온라인일 때 15초마다
polling합니다. 숨겨지거나 오프라인이면 멈춥니다. GET이나 POST 응답은 현재
축제 회차·아티스트와 요청 세대에 맞는지 확인해 오래된 응답이 최신 상태를 덮지 않게 합니다.
누적 count는 이미 표시한 값보다 오래된 응답으로 낮추지 않습니다.

라인업에는 아티스트의 간결한 count만 표시합니다. 상세의 클릭 문구는 한국어
`기대돼요 {count}`, 영어 `Hyped {count}`로 표시합니다. 영어 이외의 추가 번역은 추측하지
않습니다. `hypedEnabled`가 `false`일 때도 count는 남기고 읽기 전용으로 표시합니다.

오류 응답은 `{ error, meta }`입니다. `404 NOT_FOUND`는 등록된 아티스트를 찾지 못한 경우,
`409 HYPED_CLOSED`는 참여 시간이 아닌 경우, `429 RATE_LIMITED`는 서버 속도 제한에 걸린
경우입니다. API가 거절한 요청으로 count를 올리지 않으며, 공개 참여 API의 속도 제한은
운영 설정으로 조정합니다.
