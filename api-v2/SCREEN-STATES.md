...
|---|---|---|
| HOME | config + crowding + notices | 공지 없음이어도 전체보기 유지. 혼잡도 최초 실패는 확인 불가, 갱신 실패는 마지막 정상 상태 유지. welcomeDay=null은 공개 보류. 언어 저장은 브라우저 |
...
| STAMP-START | stamp-guide + 당일 로컬 상태 | 선택 수령 안내·QR 누락은 자료 대기. START만으로 적립하지 않음. START 전 기본 카메라 진입 정책 미정 |
| STAMP-COLLECT | guide + 로컬 count/date/started/claimed | 처음0칸. 공통 QR1회당1개·하루4개. 자정 초기화. 카메라 오류는 HTTP 오류와 분리 |
| STAMP-REWARD | 로컬4칸·수령 상태 | 4칸 전 수령 불가. 현장 담당자가 휴대폰에서 처리. 서버 지급 기록·재고·엄격한 중복 차단 없음 |
...
| ADM-GOODS-PRODUCT-EDIT | products/{id}, POST/PUT products | 이름·가격 검증. 실패 시 입력·기존 이미지 유지. 기존 옵션 상태 보존. 신규 상태 기본409, 명시적 목 시나리오로 성공 검토 |
