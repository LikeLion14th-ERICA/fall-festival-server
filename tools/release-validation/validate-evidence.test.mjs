...
async function validateMalformed(contents) {
  const temporary = await mkdtemp(resolve(tmpdir(), "espero-release-evidence-malformed-"));
  const path = resolve(temporary, "evidence.json");
  try {
...
test("malformed evidence JSON never echoes sensitive input", async () => {
  const secrets = [
    "evidence-password-should-not-echo",
    "evidence-token-should-not-echo",
    "jdbc:postgresql://user:secret@db.example.invalid:5432/app"
  ];
  const result = await validateMalformed(`{"password":"${secrets[0]}","token":"${secrets[1]}","jdbc":"${secrets[2]}",`);
  assert.equal(result.status, 1);
  assert.equal(result.stdout, "");
  assert.equal(result.stderr.trim(), "error: invalid evidence JSON input");
