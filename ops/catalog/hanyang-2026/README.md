...
`goods-draft.json`의 굿즈 이미지 7종은 관리자 이미지 업로드 API로 올려 받은 `mediaId`를 전부
채웠다(2026-09-21). 업로드는 원본이 1024×1024보다 작아 비율 유지 확대·투명 여백 추가로 처리했고,
실제 상품 등록(`POST /admin/products`)은 아직 하지 않았다 — 굿즈 입금 계좌 확인 전까지는 미룬다.
미연결(unattached) 상태로 오래 두면 정리 작업 대상이 될 수 있으므로 계좌가 확정되면 바로 등록한다.

## 비워 둔 선택 값

- 지도·핀·지도 목표: 지도 이미지와 좌표가 없다. 주점존 시설 13곳은 핀 없이 `places`에만 넣었다.
- 타임테이블 축(`timetableConfig`), 반입 금지 물품, 티켓 가격·송금 시간, 스탬프 투어 날짜와 QR
